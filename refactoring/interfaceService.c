    ...
    while (1)
    {
        ...
        if (len > 0)
        {
            ...
            case PACKET_TYPE_REQ_ACK:
                ...
                if (p_recv.seqn == *(ctx->seqn_ptr))
                {                                                        // Isso só funciona se `seqn_ptr` aponta para a próxima req a ser enviada.
            ...
            case PACKET_TYPE_HEARTBEAT:
                // NOVO: Ignorar heartbeats silenciosamente (são para servidores)
                break;
    ...
    while (1)
    {
        ...
        while (!ack_received_for_current_req && retransmissions < MAX_RETRANSMISSIONS)
        {
            ...
            *(ctx->ack_recebido_ptr) = 0;
            ...
            {                                     // Verifica a flag global que client_interface_thread setará
