...
int find_or_add_client(server_state *state, struct sockaddr_in *addr) {
    ...
    if (state->client_count < MAX_CLIENTS) { // Verifica limite
...
void *handle_request(void *arg) {
    ...
    if (idx == -1) { // Erro ao adicionar cliente
    ...
    if (is_current_server_leader && !is_duplicate) { // Apenas replica se for líder e não for duplicata
        ...
        if (num_followers > 0) {
            ...
            for (int i = 0; i < ctx->state->num_known_servers; i++) {
                if (ctx->state->known_servers[i].id != ctx->state->server_id) { // Não envia para si mesmo
...
void *manage_server_role_thread(void *arg) {
    ...
    //int election_responses_received = 0; // Para o candidato
    ...
    while (1) {
        ...
        if (state->is_leader) {
            ...
            if (now - last_heartbeat_sent_time >= HEARTBEAT_INTERVAL_SEC) {
                ...
                pthread_mutex_lock(&state->known_servers_lock);
                for (int i = 0; i < state->num_known_servers; i++) {
                    if (state->known_servers[i].id != state->server_id) { // Não envia heartbeat para si mesmo
