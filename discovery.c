...
int discovery_send_broadcast(int sock, uint16_t port, struct sockaddr_in *serveraddr) {
    ...
    struct sockaddr_in client_addr = {
        ...
        .sin_addr.s_addr = htonl(INADDR_ANY)};  // Bind to any address
    ...
    while (1) {
        ...
        ssize_t len = recvfrom(sock, &resp, sizeof(resp), 0, (struct sockaddr *)serveraddr, &addrlen);
        if (len > 0) {
            printf("Received response: type=%d, client_id=%u\n", resp.type, resp.client_id);
        } else {
